...
from utils import cal_performance, normalize_offset, cal_actionness_performance, CALF_matching
from eval import evaluate
from eval_BAA import evaluate_BAA

# Segmentation loss is forced to CE. Just cause
def train_dual(args, model, train_loader, val_loader, optimizer, scheduler, criterion,
    ...
    model_head2_sizes = [args.jointtrain["num_classes"] + 1, args.jointtrain["num_classes"] + 1 - 1*args.actionness]
    best_mAP = best_mAP
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, data in enumerate(train_loop):
            ...
            optimizer.zero_grad()
            features, past_label, trans_off_future, trans_future_target, target_actionness, dataset_num = data
            ...
            if args.seg :
                ...
                all_n_seg_total = 0
                all_n_seg_correct = 0
                # Manually iterate through the batch, as each element can come from a different dataset
                for b in range(len(dataset_num)):
                    # Choose which part of the predictions to work with depending on dataset
                    if dataset_num[b] == 1:
                        output_seg = outputs['seg'][b,:,:model_head1_sizes[0]]
                        class_weights = torch.tensor(args.class_weights, device=device)
                    elif dataset_num[b] == 2:
                        output_seg = outputs['seg'][b,:,model_head1_sizes[0]:]
                        class_weights = torch.tensor(args.jointtrain["class_weights"], device=device)
                    T, C = output_seg.size()
                    output_seg = output_seg.view(-1, C).to(device)
                    target_past_label = past_label[b]
                    ...
                    all_loss_seg += loss_seg.item()
                    all_n_seg_total += n_seg_total
                    all_n_seg_correct += n_seg_correct
                    # Only records class-wise statistics for main dataset (Only tested with BAS or BAA)
                    if dataset_num[b] == 1:
                ...
                # Record and display auxilary task training loss and accuracy
                step_log_dict["train/seg_loss"] = all_loss_seg
                postfix_kwargs["loss_seg"] = all_loss_seg
                if all_n_seg_total == 0:
                    step_log_dict["train/seg_acc"] = 1
                    postfix_kwargs["acc_seg"] = 1
                else:
                    step_log_dict["train/seg_acc"] = all_n_seg_correct/all_n_seg_total
                    postfix_kwargs["acc_seg"] = all_n_seg_correct/all_n_seg_total
            ...
            if args.anticipate :
                ...
                all_AA_loss = 0
                all_offset_loss = 0
                all_actionness_loss = 0
                all_actionness_stats = {"TP": 0, "FP": 0, "TN": 0, "FN": 0}
                # Manually iterate through the batch, as each element can come from a different dataset
                for b in range(len(dataset_num)):
                    ########################################
                    ...
                    # Choose which part of the predictions to work with depending on dataset
                    if dataset_num[b] == 1:
                        output = outputs['action'][b,:,:model_head1_sizes[1]]
                        class_weights = torch.tensor(args.class_weights, device=device)
                        class_weights[0] = args.eos_weight  # Replace background weight with EOS weight
                    elif dataset_num[b] == 2:
                        output = outputs['action'][b,:,model_head1_sizes[1]:]
                        class_weights = torch.tensor(args.jointtrain["class_weights"], device=device)
                        class_weights[0] = args.jointtrain.get("eos_weight", args.eos_weight)  # Replace background weight with EOS weight
                    T, C = output.size()
                    # Perform CALF matching of predictions and ground truth if needed
                    if args.CALF_matching:
                        raise NotImplementedError("CALF matching is not yet implemented with joint training")
                    output = output.view(-1, C).to(device)
                    target_b = target[b].contiguous().view(-1)
                    ...
                    target_b = torch.where(target_b == pad_idx, target_b, target_b - 1) if use_actionness or BCE_with_actionness else target_b       # If using Actionness, then it moves target values back by one to compensate for lack of EOS
                    # Calculate prediction loss and accuracy
                    if use_actionness or BCE_with_actionness:
                        loss, n_correct, n_total, class_stats = cal_performance(output, target_b, pad_idx, loss_func=loss_func, class_weights=class_weights[1:], actionness=True)
                    else:
                        loss, n_correct, n_total, class_stats = cal_performance(output, target_b, pad_idx, loss_func=loss_func, class_weights=class_weights, actionness=use_actionness)
                    ...
                    all_AA_acc += acc
                    all_AA_loss += loss.item()

                    ########################################
                    # Offset processing
                    ########################################
                    # Choose which part of the offset to work with depending on dataset
                    if dataset_num[b] == 1:
                        output_off = outputs['offset'][b,:args.n_query]
                    elif dataset_num[b] == 2:
                        output_off = outputs['offset'][b,args.n_query:]
                    # Normalize offset to be between 0-1 instead of 0-num_frames
                    transformed_output_off = normalize_offset(output_off, trans_off_future_mask[b], num_pred_frames)
                    transformed_target_off = normalize_offset(target_off[b], trans_off_future_mask[b], num_pred_frames)
                    # Calculate offset loss
                    if torch.sum(trans_off_future_mask[b]) == 0:
                        ...
                        torch.sum(trans_off_future_mask[b])
                    loss_off *= offset_loss_weight
                    losses += loss_off
                    epoch_loss_off += loss_off.item()
                    all_offset_loss += loss_off.item()

                    ########################################
                    # Actionness processing
                    ########################################
                    if use_actionness:
                        # Choose which part of the actionness to work with depending on dataset
                        if dataset_num[b] == 1:
                            output_actionness = outputs['actionness'][b,:args.n_query]
                        elif dataset_num[b] == 2:
                            output_actionness = outputs['actionness'][b,args.n_query:]
                        output_actionness = output_actionness.view(-1).to(device)
                        target_actionness_b = target_actionness[b].contiguous().view(-1)
                        # Calculate actionness loss
                        actionness_loss, actionness_stats = cal_actionness_performance(output_actionness, target_actionness_b, threshold=0.5)
                        losses += actionness_loss
                        epoch_loss_actionness += actionness_loss.item()
                        all_actionness_loss += actionness_loss.item()
                        for stat_key in actionness_stats.keys():
                            all_actionness_stats[stat_key] += actionness_stats[stat_key]
                    # Record class-wise stats only for main dataset (Only tested with BAS and BAA)
                    if dataset_num[b] == 1:
                ...
                step_log_dict["train/CE_acc"] = all_AA_acc/len(dataset_num)
                step_log_dict["train/CE_loss"] = all_AA_loss
                step_log_dict["train/offset_loss"] = all_offset_loss
                if use_actionness:
                    step_log_dict["train/actionnes_loss"] = all_actionness_loss
                    step_log_dict["train/actionness_acc"] = (all_actionness_stats["TP"] + all_actionness_stats["TN"]) / (all_actionness_stats["TP"] + all_actionness_stats["FN"] + all_actionness_stats["FP"] + all_actionness_stats["TN"])
                postfix_kwargs["loss_CE"] = all_AA_loss
                postfix_kwargs["loss_offset"] = all_offset_loss
                postfix_kwargs["acc_ant"] = all_AA_acc/len(dataset_num)
                if use_actionness:
                    postfix_kwargs["loss_actionness"] = all_actionness_loss
                    postfix_kwargs["acc_actionness"] = (all_actionness_stats["TP"] + all_actionness_stats["TN"]) / (all_actionness_stats["TP"] + all_actionness_stats["FN"] + all_actionness_stats["FP"] + all_actionness_stats["TN"])
        ...
        with torch.no_grad():
            for j, data in enumerate(val_loop):
                ...
                if args.seg :
                    ...
                    # Record and display auxilary task training loss and accuracy
                    step_log_dict["val/seg_loss"] = all_loss_seg
                    postfix_kwargs["loss_seg"] = all_loss_seg
                    if n_seg_total == 0:
                        step_log_dict["val/seg_acc"] = 1
                        postfix_kwargs["acc_seg"] = 1
                    else:
                        step_log_dict["val/seg_acc"] = all_n_seg_correct/all_n_seg_total
                        postfix_kwargs["acc_seg"] = all_n_seg_correct/all_n_seg_total
                ...
                if args.anticipate:
                    ...
                    for b in range(len(dataset_num)):
                        ...
                        # Perform CALF matching of predictions and ground truth if needed
                        if args.CALF_matching:
                            raise NotImplementedError("CALF matching not implemented with joint-training")
                        T, C = output.size()
                        output = output.view(-1, C).to(device)
                        target_b = target[b].contiguous().view(-1)
                        # Shift target by 1 if using actionness, so that it matches the predictions that do not have background/eos class
                        # Does not shift padding predictions
                        target_b = torch.where(target_b == pad_idx, target_b, target_b - 1) if use_actionness or BCE_with_actionness else target_b
                        # Calculate prediction loss and accuracy
                        if use_actionness or BCE_with_actionness:
                            loss, n_correct, n_total, class_stats = cal_performance(output, target_b, pad_idx, loss_func=loss_func, class_weights=class_weights[1:], actionness=True)
                        else:
                    ...
                    # Record and display main task training statistics
                    step_log_dict["val/CE_acc"] = all_AA_acc/len(dataset_num)
                    step_log_dict["val/CE_loss"] = all_AA_loss
                    step_log_dict["val/offset_loss"] = all_offset_loss
                    if use_actionness:
                        step_log_dict["val/actionnes_loss"] = all_actionness_loss
                        step_log_dict["val/actionness_acc"] = (all_actionness_stats["TP"] + all_actionness_stats["TN"]) / (all_actionness_stats["TP"] + all_actionness_stats["FN"] + all_actionness_stats["FP"] + all_actionness_stats["TN"])
                    postfix_kwargs["loss_CE"] = all_AA_loss
                    postfix_kwargs["loss_offset"] = all_offset_loss
                    postfix_kwargs["acc_ant"] = all_AA_acc/len(dataset_num)
                    if use_actionness:
